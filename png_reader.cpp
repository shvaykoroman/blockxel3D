...
streaming_chunk *
allocateChunk(Memory_arena *arena)
{
    streaming_chunk *result = PushStruct(arena,streaming_chunk);
    return result;
}


void 
appendChunk(Memory_arena *arena,streaming_buffer *compData, void *chunkData, u32 length)
{
    streaming_chunk *chunk = allocateChunk(arena);
    chunk->size = length;
...
void *
allocatePixels(Memory_arena *arena,u32 width,u32 height, u32 BPP, u32 additionalData)
{
    void *result = PushSize(arena,width * height * BPP + additionalData * height);
    return result;
}
...
png_huffman
allocateHuffman(Memory_arena *arena,u32 maxCodeLengthBits)
{
    ...
    if(fileHeader)
    {
            ...
            if(chunkHeader)
            {
                ...
                if(chunkData)
                {
                    ...
                    if(FOURCC("IHDR") == chunkHeader->u32Type)
                    {
                        ...
                        decompressedPixels = (u8*)allocatePixels(arena,width,height,4, 1);
                        
                        
                        
                        imagePixels = (u8*)allocatePixels(arena,width,ihdr->height,4, 0);
                    ...
                    else if(FOURCC("IDAT") == chunkHeader->u32Type)
                    {
                        ...
                        appendChunk(arena,&compData, chunkData, chunkHeader->length);
                    }
                }
            }
        }
        ...
        while(BFINAL == 0)
        {
            ...
            else
            {
                ...
                png_huffman litLenHuffman = allocateHuffman(arena,15);
                png_huffman distHuffman = allocateHuffman(arena,15);
                if(BTYPE == 2) // NOTE(shvayko): dynamic huffman codes
                {
                    ...
                    png_huffman dictHuffman = allocateHuffman(arena,7);
...
void
imageFlipY(u8 *pixels, u32 width,u32 height)
{
    u32 centerPoint = (height + 1) / 2;
    u32 *row0 = (u32*)pixels;
